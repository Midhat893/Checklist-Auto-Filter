...
def main(page_name,relay_checkbox_key,desc_checkbox, na_checkbox, download_key):
    if "uploaded_file" not in st.session_state:
        st.warning("Please upload the checklist file from the main page first.")
        return
    else:
        uploaded_file = st.session_state["uploaded_file"]
        df = pd.read_excel(uploaded_file, sheet_name=page_name, usecols="A,B , D, E, F", skiprows=1)
        ...
        if "Description" not in df.columns or "D1" not in df.columns or "S.No" not in df.columns:
            ...
            uses_relays = st.checkbox("Does your design use relays", key=relay_checkbox_key)
                    ...
                    for idx, row in points.iterrows():
                        ...
                        if desc:
                            with st.container():
                                ...
                                checked_key = f"checkbox_{idx}"+desc_checkbox
                                na_key = f"checkbox_na_{idx}"+na_checkbox

                                checked = st.session_state.get(checked_key, False)
                                na = st.session_state.get(na_key, False)
                                
                                col1, col2 = st.columns([0.85, 0.15])
                                with col1:
                                    checkbox_states[idx] = st.checkbox(f"✅ {desc}", key=checked_key)
                                with col2:
                                    checkbox_states[f"na_{idx}"] = st.checkbox("❌ NA", key=na_key)
            ...
            for idx, checked in checkbox_states.items():
                if checkbox_states.get(f"checkbox_checked_{idx}"):
                    df.loc[idx, "D1"] = "Checked"
                elif checkbox_states.get(f"checkbox_na_{idx}"):
                    df.loc[idx, "D1"] = "NA"
